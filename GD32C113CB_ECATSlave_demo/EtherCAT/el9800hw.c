...
------
-----------------------------------------------------------------------------------------*/
#define RxTxSpiData        SPI0_ReadWriteByte
#define SELECT_SPI         gpio_bit_reset(SPI_CS_ESC_PORT, SPI_CS_ESC_PIN);
#define DESELECT_SPI       gpio_bit_set(SPI_CS_ESC_PORT, SPI_CS_ESC_PIN);
/*-----------------------------------------------------------------------------------------
------
...
-----------------------------------------------------------------------------------------*/
/* PDI 映射 */
#define ACK_ESC_INT        exti_interrupt_flag_clear(AX58100_INT_EXTI)
#define EscIsr             AX58100_INT_IRQHandler
...
-----------------------------------------------------------------------------------------*/
/* Sync0 映射 */
#define ACK_SYNC0_INT      exti_interrupt_flag_clear(AX58100_SYNC0_EXTI)
#define Sync0Isr           AX58100_SYNC0_IRQHandler
/* Sync1 映射 */
#define ACK_SYNC1_INT      exti_interrupt_flag_clear(AX58100_SYNC1_EXTI)
#define Sync1Isr           AX58100_SYNC1_IRQHandler
...
-----------------------------------------------------------------------------------------*/
/* 定时器 映射 */
#define ethercat_timer_isr TIMER1_IRQHandler
#define ACK_TIMER_INT      timer_interrupt_flag_clear(TIMER1, TIMER_INT_FLAG_UP)
/*-----------------------------------------------------------------------------------------
------
------    Configuration Bits
------
...
/* __attribute__ ((__interrupt__, no_auto_psv))是PIC单片机的中断的写法，在GD32中不需要 */
// void __attribute__ ((__interrupt__, no_auto_psv)) EscIsr(void)
void EscIsr(void)
{
    PDI_Isr();
