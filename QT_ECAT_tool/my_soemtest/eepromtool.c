       ...
       if (wkc > 0)
       {
         ...
         if((ec_slavecount >= slave) && (slave > 0))
         {
            if ((mode == MODE_INFO) || (mode == MODE_READBIN) || (mode == MODE_READINTEL))
            {
               ...
               // zjx add
               s_eeprom_info.slave_num = slave;
               s_eeprom_info.PDI_control = *(wbuf + 0x00);
               s_eeprom_info.PDI_config = *(wbuf + 0x01);
               s_eeprom_info.config_alias = *(wbuf + 0x04);
               s_eeprom_info.checksum = *(wbuf + 0x07);
               s_eeprom_info.calculated = SIIcrc(&ebuf[0]);
               s_eeprom_info.vendor_id = *(uint32 *)(wbuf + 0x08);
               s_eeprom_info.product_code = *(uint32 *)(wbuf + 0x0A);
               s_eeprom_info.revision_number = *(uint32 *)(wbuf + 0x0C);
               s_eeprom_info.serial_number = *(uint32 *)(wbuf + 0x0E);
               s_eeprom_info.mailbox_protocol = *(wbuf + 0x1C);
               s_eeprom_info.size = esize;
               s_eeprom_info.version = *(wbuf + 0x3F);
            }
...
void my_eeprom_display(char *ifname, int slave_num, char *fname, EEPROM_info_t* p_info)
{
    slave = slave_num;
    mode = MODE_INFO;
    eepromtool(ifname, slave, mode, fname);
    memcpy(p_info, &s_eeprom_info, sizeof(EEPROM_info_t));
}
